...
Supports Google Sign-In (via streamlit-oauth), email magic link, and phone SMS OTP.
"""
    ...
    if result and "token" in result:
        ...
        if id_token:
            with st.spinner("🔄 Signing in with Google…"):
                uid, email, display_name = exchange_google_token(id_token)
            if uid and email:
            ...
            st.error("❌ Could not get ID token from Google. Please try again.")
